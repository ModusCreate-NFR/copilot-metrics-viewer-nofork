...
export default defineEventHandler(async (event) => {
  ...
  if (options.isDataMocked && mockedDataPath) {
    // Use Nitro storage API to read server assets
    const assets = useStorage('assets:server');
    const data = await assets.getItem(`mock-data/${mockedDataPath}`);
    if (!data) {
      throw createError({
        statusCode: 500,
        statusMessage: `Mock data file not found: ${mockedDataPath}`
      });
    }
